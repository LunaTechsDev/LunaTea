...
function isTouchInsideFrame(win: Window_Base) {
 #if compileMV
...
function canvasToLocal(win: Window_Base): Point {
 #if compileMV
 var x = win.canvasToLocalX(TouchInput.x);
 var y = win.canvasToLocalY(TouchInput.y);
...
inline function isOpenOrVisible(win: Window_Base) {
 return win.isOpen() || win.visible;
}

inline function isOpenAndVisible(win: Window_Base) {
 return win.isOpen() || win.visible;
}
